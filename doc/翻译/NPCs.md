...
#### Dialogue functions(对话函数)

对话函数返回或操作游戏数值。它们的作用范围与[变量](#variables)一样。

这些函数支持关键字-值对作为可选参数（kwargs），形式为 `'keyword': argument`。

函数参数可以是 `d`ouble（或子表达式）、`s`trings 或 `v`[ariables](#variables)。<br/>
一些函数支持数组参数或 kwargs，用方括号 [] 表示（例如：[`d`/`v`]）。

| 功能 | 评估 | 赋值 | 范围 | 描述 |
|----------|------|--------|-------|-------------|
| armor (护甲) ('s'/'v','s'/'v')   | ✅     | ❌      | u, n     | 返回角色在特定身体部位对特定伤害类型的护甲值。参数分别为伤害类型ID和身体部位ID。                                                                      |
| attack_speed(攻击速度)()           | ✅     | ❌      | u, n     | 返回角色当前使用武器的调整后攻击速度。                                                                                                                 |
| addiction_intensity(成瘾强度) | ✅ | ❌    | u, n     | 返回角色当前指定成瘾类型的强度。参数为成瘾类型ID。                                                                                                     |
| addiction_turns(成瘾持续时间) | ✅     | ✅      | u, n     | 返回角色对指定成瘾类型剩余的持续时间（以回合计）。参数为成瘾类型ID。                                                                                   |
| characters_nearby(附近角色数量)()      | ✅     | ❌      | u, n, global | 返回附近的角色数量（包括玩家角色和NPC，不包括幻觉NPC）。                                                                                              |
| charge_count(剩余使用次数)    | ✅     | ❌      | u, n     | 返回角色物品栏中指定物品的剩余使用次数。参数为物品ID。                                                                                                  |
| coverage(覆盖率)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回角色对特定身体部位的总覆盖率。参数为身体部位ID。<br/>对于物品，返回物品的典型覆盖率。<br/><br/>示例：<br/>`"condition": { "math": [ "u_coverage('torso')", ">", "0"] }`|
| distance(距离)('s'/'v','s'/'v')    |  ✅   |   ❌  | g  | 返回两个目标之间的距离。<br/>参数为位置变量或特殊字符串（`u`、`npc`）。`u` 表示你的位置。`npc` 表示NPC的位置。<br/><br/>示例：<br/>`"condition": { "math": [ "distance('u', loc)", "<=", "50"] }`|
| effect_intensity(效果强度)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回角色效果的强度。<br/>参数为效果ID。<br/><br/>可选参数：<br/>`bodypart`: `s`/`v` - 指定要获取/设置效果强度的身体部位。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_effect_intensity('bite', 'bodypart': 'torso')", ">", "1"] }`|
| encumbrance(负重)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回角色对特定身体部位的总负重。参数为身体部位ID。<br/>对于物品，返回物品的典型负重。<br/><br/>示例：<br/>`"condition": { "math": [ "u_encumbrance('torso')", ">", "0"] }`|
| energy(能量)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回能量字符串的数值（以毫焦为单位）（请参阅 [Units](JSON_INFO.md#units)）。<br/><br/>示例：<br/>`{ "math": [ "u_val('power')", "-=", "energy('25 kJ')" ] }`|
| faction_like(派系好感度)('s'/'v')<br/>faction_respect(派系尊重度)('s'/'v')<br/>faction_trust(派系信任度)('s'/'v')    |   ✅   |   ❌  | N/A<br/>(全局)  | 返回派系对角色的好感度/尊重度/信任度值。<br/>参数为派系ID。<br/><br/>示例：<br/>`"condition": { "math": [ "faction_like('hells_raiders') < -60" ] }`|
| field_strength(场地强度)('s'/'v')    |   ✅   |   ❌  | u, n, global  | 返回区块上一个场地的强度。<br/>参数为场地ID。<br/><br/>可选参数：<br/> `location`: `v` - 指定搜索的中心位置。<br/><br/>在全局范围中，`location` 参数是必需的。<br/><br/>示例：<br/>`"condition": { "math": [ "u_field_strength('fd_blood')", ">", "5" ] }`<br/><br/>`"condition": { "math": [ "field_strength('fd_blood_insect', 'location': u_search_loc)", ">", "5" ] }`|
| has_flag(是否具有标志)('s'/'v') | ✅ | ❌ | u, n | 检查角色是否具有某个标志。用于条件判断。参数为特性ID。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_blorg", "=", "u_has_flag('MUTATION_TRESHOLD') ? 100 : 15" ] }`|
| has_trait(是否具有特性)('s'/'v')    |  ✅   |   ❌  | u, n  | 检查角色是否具有某个特性。用于条件判断。参数为特性ID。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_blorg", "=", "u_has_trait('FEEBLE') ? 100 : 15" ] }`|
| has_proficiency(是否具有熟练度)('s'/'v')    |  ✅   |   ❌  | u, n  | 检查角色是否具有某个熟练度。用于条件判断。参数为熟练度ID。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_blorg", "=", "u_has_proficiency('prof_intro_biology') ? 100 : 15" ] }`|
| has_var(是否具有变量)('v')    |  ✅   |   ❌  | g  | 检查变量是否已定义。用于条件判断。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_blorg", "=", "has_var(fancy_var) ? fancy_var : 15" ] }`|
| hp(生命值)('s'/'v')    |  ✅   |   ✅  | u, n  | 返回或设置角色的生命值。参数为身体部位ID。对于特殊值 `ALL`、`ALL_MAJOR`、`ALL_MINOR`，获取所有/主要/次要身体部位的生命值总和，或设置所有/主要/次要身体部位的生命值。<br/><br/>对于物品，返回破坏物品所需的当前伤害量。<br/><br/>示例：<br/>`"condition": { "math": [ "hp('torso')", ">", "100"] }`|
| hp_max(最大生命值)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回角色在特定身体部位的最大生命值。<br/>参数为身体部位ID。<br/> 对于物品，返回破坏物品所需的最大伤害量。<br/><br/>示例：<br/>`"condition": { "math": [ "u_hp_max('torso')", ">=", "100"] }`|
| game_option(游戏选项)('s'/'v')   |  ✅  |   ❌   | N/A<br/>(全局)  | 返回游戏选项的数值。<br/><br/>示例：<br/>`"condition": { "math": [ "game_option('NPC_SPAWNTIME')", ">=", "5"] }`|
| gun_damage(枪械伤害)('s'/'v')     |  ✅  |   ❌   | u, n  | 返回物品的枪械伤害。参数为伤害类型。对于特殊值 `ALL`，返回所有伤害类型的总和。<br/><br/>角色必须是一个物品。<br/><br/>示例：<br/>`{ "math": [ "mygun", "=", "n_gun_damage('ALL')" ] }`<br/>查看 `EOC_test_weapon_damage` 以获取完整示例|
| item_count(物品数量)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回角色物品栏中特定物品的数量。<br/>参数为物品ID。<br/><br/>示例：<br/>`"condition": { "math": [ "u_item_count('backpack')", ">=", "1"] }`|
| item_rad(物品辐射)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回具有指定标志的已穿戴物品的辐射值。<br/>参数为标志ID。<br/><br/>可选参数：<br/>`aggregate`: `s`/`v` - 指定要运行的聚合函数，以防有多个物品。有效值为 `min`/`max`/`sum`/`average`/`first`/`last`。如果未指定，默认为 `min`。<br/><br/>示例：<br/>`"condition": { "math": [ "u_item_rad('RAD_DETECT')", ">=", "1"] }`|
| melee_damage(近战伤害)('s'/'v')     |  ✅  |   ❌   | u, n  | 返回物品的近战伤害。参数为伤害类型。对于特殊值 `ALL`，返回所有伤害类型的总和。<br/><br/>角色必须是一个物品。<br/><br/>示例：<br/>`{ "math": [ "mymelee", "=", "n_melee_damage('ALL')" ] }`<br/>查看 `EOC_test_weapon_damage` 以获取完整示例|
| monsters_nearby(附近怪物数)('s'/'v'...)     |  ✅  |   ❌   | u, n, global  | 返回附近怪物的数量。接受任意数量的 `s`tring 或 `v`ariable 位置参数作为怪物ID。<br/><br/>可选参数：<br/>`radius`: `d`/`v` - 限制半径范围（rl_dist）<br/>`location`: `v` - 以此位置为中心进行搜索<br/>`attitude`: `s`/`v` - 态度过滤器。必须是 `hostile`、`friendly`、`both` 中的一个。如果未指定，则默认为 `hostile`。<br/><br/>在全局范围中，`location` 参数是必需的。<br/><br/>示例：<br/>`"condition": { "math": [ "u_monsters_nearby('radius': u_search_radius * 3)", ">", "5" ] }`<br/><br/>`"condition": { "math": [ "monsters_nearby('mon_void_maw', 'mon_void_limb', mon_fotm_var, 'radius': u_search_radius * 3, 'location': u_search_loc)", ">", "5" ] }`|
| mod_load_order(模组加载顺序)('s'/'v')  |  ✅   |   ❌  | N/A<br/>(全局)  | 返回指定模组的加载顺序。参数为模组ID。如果模组未加载，则返回 -1。|
| mon_species_nearby(附近怪物种类)('s'/'v'...)     |  ✅  |   ❌   | u, n, global  | 与 `monsters_nearby()` 相同，但参数是怪物种类 |
| mon_groups_nearby(附近怪物组)('s'/'v'...)     |  ✅  |   ❌   | u, n, global  | 与 `monsters_nearby()` 相同，但参数是怪物组 |
| moon_phase(月相)()     |  ✅  |   ❌   | N/A<br/>(全局)  | 返回当前月相。<pre>MOON_NEW = 0,<br/>WAXING_CRESCENT = 1,<br/>HALF_MOON_WAXING = 2,<br/>WAXING_GIBBOUS = 3,<br/>FULL = 4,<br/>WANING_GIBBOUS = 5,<br/>HALF_MOON_WANING = 6,<br/>WANING_CRESCENT = 7 |
| num_input(数字输入)('s'/'v','d'/'v')   |  ✅  |   ❌   | N/A<br/>(全局)  | 提示玩家输入数字。<br/>参数为提示文本、默认值：<br/>`"math": [ "u_value_to_set", "=", "num_input('Playstyle Perks Cost?', 4)" ]|
| pain(疼痛)()     |  ✅  |   ✅   | u, n  | 返回或设置疼痛值<br/> 示例：<br/>`{ "math": [ "n_pain()", "=", "u_pain() + 9000" ] }`|
| proficiency(熟练度)('s'/'v')    |  ✅   |   ✅  | u, n  | 返回或设置熟练度<br/>参数为熟练度ID。<br/><br/> 可选参数：<br/>`format`: `s` - `percent` 返回或设置学习进度的百分比。`permille` 同理。`time_spent` 返回或设置总学习时间。`time_left` 返回或设置剩余学习时间。`total_time_required` 返回学习特定熟练度所需的总时间（只读）。<br/><br/>示例：<br/>`{ "math": [ "u_proficiency('prof_intro_chemistry', 'format': 'percent')", "=", "50" ] }`|
| school_level(技能等级)('s'/'v')    |  ✅   |   ❌  | u, n  | 返回该系列已知法术的最高等级。<br/>参数为系列ID。<br/><br/>示例：<br/>`"condition": { "math": [ "u_school_level('MAGUS')", ">=", "3"] }`|
| school_level_adjustment(技能等级调整)('s'/'v')    |   ✅  |  ✅   | u, n  | 返回或设置临时的施法者等级调整值。只能由在事件 `opens_spellbook` 上触发的 EoCs 使用。在事件触发之前，旧值将重置为 0。为避免覆盖其他 EoCs 中的值，建议在此处使用 `+=` 或 `-=` 调整值，而不是设置为绝对值。<br/>参数为系列ID。<br/><br/>示例：<br/>`{ "math": [ "u_school_level_adjustment('MAGUS')", "+=", "3"] }`|
| skill(技能)('s'/'v')    |  ✅  |   ✅   | u, n  | 返回或设置技能等级<br/><br/>示例：<br/>`"condition": { "math": [ "u_skill('driving')", ">=", "5"] }`<br/>`"condition": { "math": [ "u_skill(someskill)", ">=", "5"] }`|
| skill_exp(技能经验)('s'/'v')    |  ✅  |   ✅   | u, n  | 返回或设置技能经验<br/> 参数为技能ID。 <br/><br/> 可选参数：<br/>`format`: `s`/`v` - 必须是 `percentage` 或 `raw`。<br/><br/>示例：<br/>`{ "math": [ "u_skill_exp('driving', 'format': crt_format)", "+=", "10"] }`|
| spell_exp(法术经验)('s'/'v')    |  ✅  |   ✅   | u, n  | 返回或设置法术经验<br/> 示例：<br/>`"condition": { "math": [ "u_spell_exp('SPELL_ID')", ">=", "5"] }`|
| spell_exp_for_level(升级所需法术经验)('d'/'v')    |  ✅  |   ❌   | g  | 返回升级到特定法术等级所需的经验值。 <br/> 示例：<br/>`"math": [ "spell_exp_for_level(u_spell_level('SPELL_ID')) * 5"] }`|
| spell_count(法术数量)()    |  ✅   |   ❌  | u, n  | 返回角色已学会的法术数量。<br/><br/> 可选参数：<br/>`school`: `s/v` - 返回该系列已知法术的数量。<br/><br/> 示例：<br/>`"condition": { "math": [ "u_spell_count('school': 'MAGUS')", ">=", "10"] }`|
| spell_level(法术等级)('s'/'v')    |  ✅   |   ✅  | u, n  | 返回或设置特定法术的等级。-1 表示在阅读时未知法术，如果写入则应遗忘法术。<br/>参数为法术ID。如果给定 `"null"`，则返回角色已知法术的最高等级（只读）。<br/> 示例：<br/>`"condition": { "math": [ "u_spell_level('SPELL_ID')", "==", "-1"] }`|
| spell_level_adjustment(法术等级调整)('s'/'v')    |   ✅  |  ✅   | u, n  | 返回或设置临时的施法者等级调整值。只能由在事件 `opens_spellbook` 上触发的 EoCs 使用。在事件触发之前，旧值将重置为 0。为避免覆盖其他 EoCs 中的值，建议在此处使用 `+=` 或 `-=` 调整值，而不是设置为绝对值。<br/>参数为法术ID。如果给定 `"null"`，则调整所有法术等级。<br/><br/>示例：<br/>`{ "math": [ "u_spell_level_adjustment('SPELL_ID')", "+=", "3"] }`|
| spellcasting_adjustment(法术施法调整)('s'/'v')    |   ❌  |  ✅   | u  | 临时改变法术施法属性。仅可由在事件 `opens_spellbook` 上触发的 EoCs 使用。事件触发前，旧值将重置为默认值。倍增器默认值为 1，而调整值默认值为 0。赋值函数作为对默认值的调整，赋值的值作为调整的值。因此，`=` 的功能与 `+=` 相同。不可能读取这些值，因此不可能使用 `+=`。<br/><br/>可能的参数值：<br/>`caster_level` - 调整值 - 改变给定法术的施法者等级。与 spell_level_adjustment 类似，但无法由 `math` 函数读取。<br/>`casting_time` - 倍增器 - 改变给定法术的施法时间。<br/>`cost` - 倍增器 - 改变给定法术的消耗。请注意，这不会更改法术可能消耗的物品。<br/>`aoe` - 倍增器 - 改变法术的影响范围。<br/>`range` - 倍增器 - 改变法术的范围。<br/>`duration` - 倍增器 - 改变法术的持续时间。<br/>`difficulty` - 调整值 - 改变法术的难度，从而改变施法失败的概率。<br/>`somatic_difficulty` - 倍增器 - 改变装备对施法时间和难度的影响。如果设置为 0，还将消除施法时必须手部空闲的要求。请注意，作为倍增器，它的初始值为 1，设置值实际上会调整它。因此，将值设置为 -1 将导致最终值为 0。或者，两次将其设置为 -0.5 也可以达到目的。<br/>`sound` - 倍增器 - 改变法术的响度以及嘴部装备对法术的影响。<br/>`concentration` - 倍增器 - 改变专注对法术难度的影响。<br/><br/>可选参数：<br/>`flag_blacklist`: `s/v` 和<br/>`flag_whitelist`: `s/v` - 仅对与黑名单和/或白名单匹配的法术应用修改<br/>`mod`: `s/v`、`school`: `s/v`、`spell`: `s/v` - 只能应用其中之一。限制将受影响的法术。如果未指定任何一个，修改将应用于所有法术（黑名单和白名单仍然分别应用）。<br/><br/>示例：<br/>`{ "math": [ "u_spellcasting_adjustment('casting_time', 'mod': 'magiclysm', 'flag_blacklist': 'CONSUMES_RUNES' )", "=", "-0.95" ] }`|
| value_or(变量值或默认值)('v','d'/'v')    |  ✅   |   ❌  | g  | 如果定义了变量，则返回变量的值，否则返回提供的值<br/><br/> 示例：<br/>`"condition": { "math": [ "u_blorg", "=", "value_or( fancy_var, 15 )" ] }`|
| time(时间)('s'/'v')    |  ✅   |   ✅  | N/A<br/>(全局)  | 返回时间段字符串（请参阅 [Units](JSON_INFO.md#units)）的数字值（以回合计）。<br/><br/>特殊值：<br/>`now` - 返回自回合零以来的持续时间<br/>`cataclysm` - 返回灾变和回合零之间的持续时间<br/><br/>`time('now')` 可作为赋值目标来改变当前回合。<br/><br/>可选参数：<br/> `unit`: 指定返回单位。如果未指定或为空，则默认为 `turns`。<br/><br/>示例：<br/>`{ "math": [ "time('now') - u_timer_caravan_RandEnc", ">", "time('1 h')" ] }`|
| time_since(变量)    |  ✅   |   ❌  | N/A<br/>(global)  | 便捷函数，返回自变量中存储的时间点以来的时间段的数字值（以回合计）。<br/><br/>特殊值：<br/>`cataclysm` - 返回自灾变开始以来的时间<br/>`midnight` - 返回自午夜开始的时间<br/><br/>可选参数：<br/> `unit`: 指定返回单位。如果未指定或为空，则默认为 `turns`。<br/><br/>如果参数是未定义的变量，则返回 -1。<br/><br/>示例：<br/>`{ "math": [ "time_since(u_timer_caravan_RandEnc)", ">", "time('1 h')" ] }`<br/>`{ "math": [ "time_since('cataclysm', 'unit':'years') > 1" ] }`|
| time_until_eoc(`s`/`v`)  |  ✅   |   ❌  | N/A<br/>(global)  | 返回直到下一个 EOC 调度运行的时间。参数是 EOC id。<br/><br/>可选参数：<br/> `unit`: 指定返回单位<br/><br/>如果未调度 EOC，则返回 -1。 |
| val(属性)(`s`)    |  ✅   |   varies  | u, n  | 返回或设置角色或物品值。参数是 [角色/物品方面](#list-of-character-and-item-aspects)。<br/><br/>由于历史原因，这些都在一个函数中，并且通常测试不足。如果您需要其中一个，请考虑将其转换为原生数学函数。<br/><br/>示例：<br/>`{ "math": [ "u_val('strength')", "=", "2" ] }`|
| vitamin(维生素)(`s`/`v`)    |  ✅   |   ✅  | u, n  | 返回或设置角色的维生素水平。<br/>参数是维生素 ID。<br/><br/>示例：<br/>`{ "math": [ "u_vitamin('mutagen')", "=", "0" ] }`|
| warmth(温度)(`s`/`v`)    |  ✅   |   ❌  | u, n  | 返回角色在身体部位上的温度。<br/>参数是身体部位 ID。<br/><br/>示例：<br/>在游戏中显示的值计算如下。<br/> `"{ "math": [ "u_warmth_in_game", "=", "(u_warmth('torso') / 100) * 2 - 100"] }`|
| vision_range()    |  ✅   |   ❌  | u, n  | 返回角色的视野范围，经过突变、效果和其他问题调整。<br/><br/>示例：<br/> `"{ "math": [ "u_vision_range", "<", "30"] }`|
| weather(天气属性)(`s`)  |  ✅  |   ✅   | N/A<br/>(global)  | 返回或设置天气方面<br/><br/>方面必须是以下之一：<br/>`temperature`（以开尔文为单位），<br/>`humidity`（以百分比表示），<br/>`pressure`（以毫巴为单位），<br/>`windpower`（以英里/小时为单位）。<br/>`precipitation`（以 mm/h 表示），值为 0.5（非常轻微）、1.5（轻微）或 3（大量）。只读。<br/><br/>温度转换函数可用：`celsius()`、`fahrenheit()`、`from_celsius()` 和 `from_fahrenheit()`。<br/><br/>示例：<br/>`{ "math": [ "weather('temperature')", "<", "from_fahrenheit( 33 )" ] }`<br/>`{ "math": [ "fahrenheit( weather('temperature') )", "==", "21" ] }`|
| damage_level()    |  ✅   |   ❌  | u, n  | 返回发出者的伤害等级，说话者必须是一个物品。<br/><br/>示例：<br/>`"condition": { "math": [ "n_damage_level()", "<", "1" ] }`|
